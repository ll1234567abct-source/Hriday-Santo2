...
module.exports = {
  ...
  onStart: async function ({ api, event }) {
    ...
    try {
      ...
      const cachePath = cacheFolder + "/premiuminfo.gif";
      ...
      // COMMAND COUNT
      const commandCount =
        global.GoatBot?.commands?.size || 6;
      ...
      for (let frame = 0; frame < 45; frame++) {
        ...
        borderLights.forEach((light, i) => {
          ...
          ctx.shadowBlur = 20;
          ctx.shadowColor =
            ledColors[(frame + i) % ledColors.length];
        ...
        commands.forEach((cmd, index) => {
          ...
          const x = startX + (index % 3) * 205;
          const y2 =
            startY + Math.floor(index / 3) * 75;

          ctx.fillStyle = "#0d0d0d";
          roundRect(ctx, x, y2, 180, 55, 16, true);

          ctx.strokeStyle =
            ledColors[(index + frame) % ledColors.length];

          ctx.lineWidth = 3;

          roundRect(ctx, x, y2, 180, 55, 16, false);
